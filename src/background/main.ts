// PowerChat Background Service Worker
// Queue/state management per tab

import type { QueueItem, MessageType } from '../shared/types';
...
const keyFor = (tabId: number): string => `queue:${tabId}`;
...
async function getState(tabId: number): Promise<TabState> {
  ...
  if (!s) {
    const saved = (await chrome.storage.local.get(keyFor(tabId)))[keyFor(tabId)] as
      | { queue?: QueueItem[]; paused?: boolean }
      | undefined;
    s = {
      queue: saved?.queue ?? [],
...
async function broadcast(tabId: number): Promise<void> {
  const s = await getState(tabId);
  try {
...
async function maybeSendNext(tabId: number): Promise<void> {
  const s = await getState(tabId);
...
chrome.runtime.onMessage.addListener(
  (msg: MessageType & { tabId?: number }, sender, sendResponse: (response?: unknown) => void) => {
